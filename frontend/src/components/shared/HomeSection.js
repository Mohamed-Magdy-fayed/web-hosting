...
const HomeSection = ({ children, imgUrl, imgAlt, imageFirst, darkerBG, centeredTitle, hero, mobileImgSecond }) => {
    return (
        <section className={`${darkerBG ? 'bg-gray-100' : ''} ${hero ? 'min-h-screen' : ''} w-full grid grid-cols-1 pb-24 pt-24 ${imgUrl ? 'lg:grid-cols-2' : ''} gap-10 px-12 md:px-16 lg:px-20 xl:px-24 2xl:px-32`}>
            ...
            {imgUrl && (
                <div className={`flex justify-center ${imageFirst ? 'order-2 lg:order-1' : 'order-1 lg:order-2'} ${hero && 'mt-8 '} lg:mt-0`}>
                    <img
